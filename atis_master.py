...
def wind_components_summary(full_text, runways_in_use):
    ...
    if wind_speed == 0:
        return "Wind Components: Calm - no crosswind component."
    if wind_dir is None or wind_speed is None:
        return "Wind Components: Could not parse wind data from ATIS."
    lines = [f"Wind Components (winds {wind_dir:03d} at {wind_speed}kt):"]
...
def run_atis_monitor():
    ...
    try:
        ...
        if current_letter == "Unknown":
            print("Could not parse ATIS letter. Skipping notification.")
            ...
            message = (
                f"*KDVT ATIS - Information {current_letter}*\n\n"
                f"{full_text}\n\n"
                f"{wind_summary}"
            )
        ...
        try:
            ...
            print(f"Could not delete Gemini file: {e}")

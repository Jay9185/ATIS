...
def parse_wind(full_text):
    ...
    Handles formats like: '270 at 8', '27008KT', 'calm'
    Returns (direction_deg, speed_kt) or (None, 0) if calm/unparseable.
    """
    # Match "270 at 8" or "270 at 8 gusting 15" style
    m = re.search(r'wind[s]?\s+(\d{3})\s+at\s+(\d+)', full_text, re.IGNORECASE)
    if m:
        return int(m.group(1)), int(m.group(2))

    # Match METAR-style "27008KT" or "27008G15KT"
...
def wind_components_summary(full_text, runways_in_use):
    ...
    for rwy in runways_in_use:
        heading = RUNWAY_HEADINGS.get(rwy)
        if heading is None:
...
def parse_runways(full_text):
    """
    Extract runways in use from the ATIS text.
    Matches patterns like: 'Runway 7L', 'Runways 7L and 25R', 'landing runway 25R'
    Returns a list of runway strings e.g. ['7L', '25R']
    """
    matches = re.findall(r'\bRunway[s]?\s+([\d]{1,2}[LRC]?(?:\s+and\s+[\d]{1,2}[LRC]?)*)',
                         full_text, re.IGNORECASE)
    runways = []
    for match in matches:
        # Split on 'and' to handle "7L and 25R"
...
def save_transcript(timestamp, letter, full_text, runways, wind_summary):
    ...
    with open(filename, "w") as f:
        ...
        f.write(full_text)
        f.write(f"\n{'-' * 40}\n")
        f.write(wind_summary.replace("*", ""))  # Strip Markdown for plain text file
...
def run_atis_monitor():
    ...
    try:
        ...
        RUNWAYS IN USE: [List all active runways, e.g. 7L, 25R]
        NOTICES: [Any NOTAMs or special notices, or 'None']

        Then provide the full verbatim ATIS text below the fields.
        """
        ...
        match = re.search(r'LETTER:\s*([A-Z][a-z]*)', full_text)
        if match:
            current_letter = match.group(1).strip()

        # 4. Parse active runways from Gemini's structured output first,
        #    then fall back to scanning the full text
        runways_in_use = []
        rwy_line = re.search(r'RUNWAYS IN USE:\s*(.+)', full_text, re.IGNORECASE)
        if rwy_line:
            runways_in_use = re.findall(r'\b(\d{1,2}[LRC]?)\b', rwy_line.group(1).upper())
        if not runways_in_use:
            runways_in_use = parse_runways(full_text)
